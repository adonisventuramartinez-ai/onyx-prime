...
export default function Home() {
  ...
  useEffect(() => {
    setPaso("1. Iniciando fetch...");
    
    fetch("/api/peliculas")
      .then((res) => {
        setPaso(`2. Respuesta HTTP: ${res.status}`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
      })
      .then((data) => {
        setPaso("3. Datos recibidos");
        const pelis = data.peliculas || data || [];
        setPeliculas(pelis);
        setCargando(false);
        setPaso(`✅ ${pelis.length} películas cargadas`);
      })
      .catch((err) => {
        setPaso(`❌ Error: ${err.message}`);
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[#141414] text-white flex items-center justify-center p-4">
        <div className="bg-red-500/20 border border-red-500 rounded-lg p-6 max-w-lg w-full">
          <h2 className="text-xl font-bold text-red-400 mb-2">❌ Error</h2>
          <p className="text-gray-300 mb-2">{error}</p>
          <p className="text-gray-500 text-sm">Paso: {paso}</p>
          <button 
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {peliculas.slice(0, 20).map((p) => (
                <div key={p.id} className="group cursor-pointer transition-all duration-300 hover:scale-105">
